...
public class MedialistModule extends ReactContextBaseJavaModule {

    private final ReactApplicationContext reactContext;

    public MedialistModule(ReactApplicationContext reactContext) {
        super(reactContext);
    ...
    private final String[] metadatas = { "album", "album_artist", "comment", "copyright", "creation_time", "disc",
            "encoder", "encoded_by", "genre", "language", "performer", "publisher", "service_name", "service_provider",
            "track", "variant_bitrate", "icy_metadata", "framerate", "chapter_start_time", "chapter_end_time", "artist",
            "composer", "title", "date", "duration", "rotation" };
    ...
    private void getMetadata(String path, ReadableMap options, Promise promise) {
        ...
        try {
            ...
            // get all metadata - TODO :: Loop through only values for audio or video
            // metadata based on media type for a small performance boost
            for (String meta : metadatas) {
                putString(result, meta, mmr.extractMetadata(meta));
            }

            if (result.hasKey("framerate") && !result.hasKey("rotation")) {
                putString(result, "rotation",
            ...
            if (options.getBoolean("getThumb")) {
                ...
                if (bmp != null) {
                    // Bitmap bmp2 = mmr.getFrameAtTime((long) 4E6,
                    // FFmpegMediaMetadataRetriever.OPTION_CLOSEST);
                    // if (bmp2 != null) bmp = bmp2;

                    /*
                     * The image returned seems to be always in landscape mode and does not follow
                     * the rotation of the video. get the rotation from the metadata and apply the
                     * correction so the image is straight.
                     */
